...
- Iteration 4: Added `EnemyPool`, `EnemySpawner`, and `EnemyPoolMember`; updated `UnitMovement` goal-reached flow to prefer pooled despawn (fallback keeps previous destroy/disable behavior) and updated `UnitHealth` runtime reset on `OnEnable` so pooled enemies respawn with valid state.
- Iteration 5: Added event-driven damage feedback in `UnitHealth` (`DamageTaken` + `GlobalDamageTaken`) and layered status-safe hit flash/burn tick feedback in `UnitEffects`. Added runtime-bootstrapped pooled `EnemyHealthBarSystem` for damage-time bars updated from global damage events.
- Iteration 6: Added scene-wired tower selection + data-driven tower upgrade flow (`TowerSelectionService`, `TowerSelectionInput`, `TowerUpgradable`, `TowerUpgradeTree`) and UI presenter for portrait mobile upgrade panel with upgrade/sell actions.
